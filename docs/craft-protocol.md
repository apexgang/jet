...
The concrete SDK currently implements Craft 1.10 and specification 1.0. Craft 1.1 adds managed Run supervision through the `runs` capability; see [Managed Run execution](managed-runs.md). Craft 1.2 adds recovery, Craft 1.3 adds turn and file-change evidence, Craft 1.4 adds the `fork` feature and capability, Craft 1.5 adds structured Conversation, Run, and Managed-process titles, Craft 1.6 adds the `remote_tools` feature and Jet remote tool calls, Craft 1.7 adds the `usage` event; see [Usage records](usage-records.md), and Craft 1.8 adds the structured approval request a host reviews or shows to a person. Craft 1.9 adds constrained child work. Craft 1.10 adds explicit native Model selection and Model-pinned resume. Craft 1.0 turns and actions remain supported. There is no previous released Craft major yet; the negotiation conformance test exercises current/previous-major offers with different minor ceilings. Adding a future codec major requires an implementation as well as a declaration. Advertising a new major in TOML cannot make this SDK speak it.
...
Craft 1.7 may report `usage`: what the Harness said it consumed, or how full a Provider quota window is, in Jet's vocabulary. It changes no Run state and grants nothing, the host stamps the time and the Account binding it belongs to, and the complete native event still travels beside it. See [Usage records](usage-records.md).

Craft 1.8 may report `{"kind":"approval_requested","request":{"request_id":"perm-1","tool":"Write","action":"{\"file_path\":\"note.txt\"}"}}` before the `waiting_for_approval` activity, so a host never has to guess which request a waiting Run is waiting on. The request describes what the Harness asked for; it grants nothing, and the Craft keeps holding the native request until the host answers it with an approval action. `tool` is at most 128 bytes and `action` at most 4096, cut on a character boundary. See [Automatic review](automatic-review.md).

Craft 1.5 may report `conversation_title`, `run_title`, or `process_title`. The first two are automatic naming candidates and cannot replace a manual name. A process title names the exact PID from `run_started` and updates only that Managed process's live label; it never changes the Conversation or Run name.
...
## Model-pinned continuation

Craft 1.10 reports `{"kind":"model","model":"resolved-model-id"}` from the native session's explicit selection. Per-model consumption and child-work usage do not select the session Model. The host retains this observation with the acknowledged source batch.

A 1.10 `resume` may include `model`. A Craft accepting that field must enforce the exact Model while reopening `native_conversation`. It must fail if it cannot enforce the selection. Older contracts cannot accept a Model-pinned resume. [Auto-continue](auto-continue.md) keeps the input pending when the native identity, resolved Model, or required resume support is unavailable.
