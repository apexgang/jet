...
| Harness approval | `run_execution` with `waiting_for_approval`, plus structured Event Presentation | No generic approval-decision Command exists | Display is safe. Do not route the choice through a local callback or raw Harness channel | Backend dependency: `approval_decision_command` |
