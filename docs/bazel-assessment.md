...
| Generated contracts | [Just recipes](../packages/justfile) compile Rust schema emitters, run [Python generation](../.github/scripts/wire-models.py), and check fixtures in Rust, TypeScript, and Swift. | This is the clearest current seam where one declared build graph could improve dependency tracking. |
