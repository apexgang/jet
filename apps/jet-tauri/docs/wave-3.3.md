...
same Command ID ("Try again"), even after the dialog is closed and reopened.
...
The spec granted Trash commands and name lookups to both windows. Each
command is granted only to a window with a call site, following AGENTS.md. The
manifest test in `src-tauri/src/jet/mod.rs` pins the split. Recovery, the
