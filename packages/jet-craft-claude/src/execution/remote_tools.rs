...
impl RemoteTools {
	...
	pub(crate) fn observe(&mut self, event: &Value) -> Option<Observed> {
		if event["type"] != "control_request"
			...
			|| event["request"]["server_name"]
				!= crate::execution::harness::SERVER
		{
			return None;
		}
		...
		if message["method"] == "tools/list" {
			let mut tools = crate::execution::approval::tools();
			...
			return Some(Observed::Reply(crate::execution::approval::reply(
				id, &call, &tools,
			)));
		}
	...
	pub(crate) fn reply(
		...
		Ok(crate::execution::approval::reply(
			&id,
			&call,
			&json!({
...
fn invalid(id: &str, call: &Value) -> String {
	crate::execution::approval::reply(
		id,
		call,
