...
mod tests {
	...
	use crate::filesystem::relative_path::{GrantedRoot, RelativePath};
	...
	fn a_root_is_used_only_while_it_is_still_the_directory_that_was_granted() {
		...
		let through_alias =
			GrantedRoot::verify(&alias).map_err(|error| error.code);
		std::fs::rename(&root.canonical, root.dir.path().join("moved"))
			.unwrap();
		let gone =
