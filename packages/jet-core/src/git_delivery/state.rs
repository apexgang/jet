...
pub(crate) async fn query(
	...
	tx.git_deliveries(id.0)
		...
		.into_iter()
		.map(|json| {
			crate::run::state::decode::<Document>(&json).map(|doc| doc.delivery)
		})
		.collect()
}
...
pub(crate) async fn admit(
	...
	let source = match checkpoint {
		Some(source) => {
			let checkpoint: ChangeCheckpoint = crate::run::state::decode(
...
async fn queue(
	...
	crate::git_delivery::io::validate_operation(&operation)?;
	if matches!(
		operation,
	...
	let observed = crate::git_delivery::io::inspect(&root, &operation).await;
	...
	let utility_job = if matches!(
		...
		match crate::utility::work::admit(
			tx,
			actor,
			command_id,
		...
		&& let Some(checkpoint) = checkpoint
	{
		...
		if doc.head != checkpoint.after.commit {
			let current_tree = crate::git_delivery::io::git(
				&doc.root,
				&["rev-parse", "HEAD^{tree}"],
			)
			.await;
		...
		{
			let tree = crate::git_delivery::io::git(
				&doc.root,
				&[
					"rev-parse",
		...
		if !checkpoint.after.omitted_files.is_empty()
			|| crate::checkpoint::pressure::incomplete(&checkpoint.before)
			|| crate::checkpoint::pressure::incomplete(&checkpoint.after)
			|| dirty_baseline
		{
			doc.delivery.outcome = GitDeliveryOutcome::Failed {
