...
impl Core {
				...
				.write(async |tx| {
					...
					let result = match &doc.delivery.operation {
						...
						GitOperation::DraftPullRequest { .. } => {
							crate::git_delivery::github::apply(
	...
	async fn prepare_git(&self, doc: &mut Document) -> Result<(), CoreError> {
		...
		self.store
			.read(async |tx| {
				...
				if let Some(id) = doc.delivery.utility_job {
					match crate::utility::work::query(tx, id).await?.outcome {
						UtilityOutcome::Text {
							text,
							body,
							fallback_reason,
		...
		match &doc.delivery.operation {
			...
			GitOperation::DraftPullRequest { .. } => {
				...
				crate::git_delivery::github::preflight(
					self.github_host.as_ref(),
					doc,
				)
				.await?;
		...
		if let Some(tree) = &doc.tree {
			let current = crate::workspace::with_scratch(
				...
				async |scratch| {
					...
					let index =
						crate::workspace::tree_capture::ScratchIndex::new(
							&doc.root,
							&path,
	...
	async fn reconcile_git(&self, doc: &Document) -> GitDeliveryOutcome {
		...
		let done = match &doc.delivery.operation {
			...
			GitOperation::DraftPullRequest { .. } => {
				return match crate::git_delivery::github::reconcile(
					self.github_host.as_ref(),
					doc,
				)
				.await
...
async fn completed(
	...
	match (
		crate::workspace::worktree::resolve_commit(&doc.root, "HEAD").await,
