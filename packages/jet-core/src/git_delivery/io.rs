...
pub(crate) async fn git(
	...
	run(
		crate::project::repository::command(root).args(args),
		&[],
		65536,
	)
	.await
...
pub(crate) async fn inspect(
	...
	if tokio::fs::canonicalize(root)
		...
		!= root
		|| crate::project::repository::verdict(root).await?
			!= crate::project::repository::Verdict::Registrable
	{
	...
	Ok(RepositoryState {
		head: crate::workspace::worktree::resolve_commit(root, "HEAD").await?,
...
pub(crate) async fn prepare_commit(
	...
	run(
		crate::project::repository::command(&doc.root).args([
			"commit-tree",
			tree,
			"-p",
			&doc.head,
...
pub(crate) async fn commit(
	...
	crate::workspace::with_scratch(home, "delivery", async |scratch| {
		...
		let captured = crate::workspace::tree_capture::ScratchIndex::new(
			&doc.root,
			&temporary,
...
pub(crate) async fn push(
	...
	let mut command = crate::project::repository::command(&doc.root);
	command.args([
		"-c",
