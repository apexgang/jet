...
pub(crate) struct Entry {
	...
	#[serde(default, skip_serializing_if = "Option::is_none")]
	pub(crate) schedule: Option<crate::schedule::work::ScheduleInput>,
	pub(crate) turn: Turn,
...
pub(crate) enum Admission {
	Scheduled {
		prompt: String,
		schedule: crate::schedule::work::ScheduleInput,
	},
	Prompt(String),
	Review {
		prompt: String,
...
impl Admission {
	fn into_parts(
		...
		Option<crate::schedule::work::ScheduleInput>,
	) {
		match self {
...
pub(crate) async fn load(
	...
	tx.turn_queue(id.0)
		.await?
		.map(|json| {
			crate::run::state::decode::<StoredQueue>(&json)
...
pub(crate) async fn commit(
	...
	{
		crate::auto_continue::work::changed(tx, actor, id, retry.clone(), now)
			.await?;
	}
