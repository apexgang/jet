...
use crate::{
	...
	TurnState, turn::queue as turn_queue,
};
	...
	pub(crate) async fn dispatch_turn(
		...
		let next = self
			...
			.write(async |tx| {
				...
				if !crate::schedule::work::can_dispatch(
					tx,
					id,
					&queue,
					self.now_unix_ms(),
				...
				if !crate::auto_continue::work::matches_execution(
					tx,
					&queue.entries[0],
					&plan,
					&state,
				...
				if let Some(entry) = &next {
					crate::auto_continue::work::claimed(
						tx,
						id,
						&mut queue,
						entry,
					...
					if state.changes.is_some() {
						crate::checkpoint::state::observe(
							...
							tx,
							run_id,
							&crate::run::state::Observation::TurnStarted,
						)
						.await?;
					}
