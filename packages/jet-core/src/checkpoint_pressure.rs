...
pub(crate) async fn snapshot(root: &Path) -> Result<ChangeSnapshot, CoreError> {
	let commit = crate::worktree::resolve_commit(root, "HEAD").await?;
	let tree =
		crate::repository::git(root, &["rev-parse", "--verify", "HEAD^{tree}"])
			.await?;
	if !tree.status.success() {
		return Err(crate::change_artifact::failed(tree.stderr));
	}
	...
	Ok(ChangeSnapshot {
		...
		omitted_files: crate::checkpoint_omissions::inspect(root, 0).await?,
...
pub(crate) async fn capture_bytes(root: &Path) -> Result<u64, CoreError> {
	...
	if !output.status.success() {
		return Err(crate::change_artifact::failed(output.stderr));
	}
	let paths: std::collections::HashSet<_> =
		output.stdout.split('\0').collect();
	let files = crate::checkpoint_omissions::inspect(root, 0).await?;
