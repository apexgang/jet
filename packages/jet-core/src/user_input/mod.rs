...
use crate::{
	...
	RunId, WorkspaceId,
	event::{EventSequence, EventSubject},
	filesystem::relative_path::RelativePath,
	user_input::files::{
		observe, revision, root, valid_revision, write_atomic,
	},
};
use jet_store::{NewUserEditIntent, ReadTransaction, WriteTransaction};
...
pub(crate) async fn apply(
	...
	if !no_change {
		...
		if !recovered && let Some((run_id, window)) = change_run {
			...
			match window {
				crate::checkpoint::change_evidence::Window::ActiveTurn => {
					crate::checkpoint::change_evidence::record(
						core, tx, run_id, evidence,
					)
					.await?;
				}
				crate::checkpoint::change_evidence::Window::BetweenTurns => {
					crate::checkpoint::change_evidence::record_between_turns(
						core, tx, run_id, evidence,
					)
					.await?;
				}
...
async fn subject_and_run(
	...
	(
		EventSubject,
		Option<(RunId, crate::checkpoint::change_evidence::Window)>,
	),
	CoreError,
> {
