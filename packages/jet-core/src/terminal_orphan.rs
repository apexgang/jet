...
use crate::{
	...
	OrphanedExecution, RunId, TerminalId, TerminalOperation, terminal,
};
...
impl Core {
	pub(crate) async fn is_terminal_orphan(
		...
		let json = self
			...
			.await?;
		Ok(json
			.map(|json| crate::run_state::decode::<OrphanedExecution>(&json))
			.transpose()?
...
pub(crate) async fn settle(
	...
	let orphan: OrphanedExecution = crate::run_state::decode(&json)?;
