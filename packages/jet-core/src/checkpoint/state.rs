...
use crate::{
	...
	checkpoint::{capture as checkpoint_capture, change_artifact},
	run::state::{self as run_state, Observation, State},
};
...
pub(crate) async fn observe(
	...
	if let Observation::FileChanged(evidence) = observation {
		...
		return crate::checkpoint::change_evidence::record(
			core,
			tx,
			run_id,
			evidence.clone(),
	...
	if !tracking.evidence_incomplete {
		crate::checkpoint::change_evidence::attribute(
			&mut files,
			&tracking.evidence,
		);
	}
	let artifact = if crate::checkpoint::pressure::incomplete(&before)
		|| crate::checkpoint::pressure::incomplete(&after)
	{
		crate::checkpoint::pressure::artifact()
	} else {
	...
	.await?;
	crate::git_delivery::state::automate(tx, &checkpoint, plan.client_id)
		.await?;
