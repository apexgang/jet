...
use crate::{
	...
	RunId,
	checkpoint::change_artifact,
	project::repository,
	workspace::{self, tree_capture, worktree},
};
use std::path::Path;
...
pub(crate) async fn snapshot(
	...
	let capture_bytes =
		crate::checkpoint::pressure::capture_bytes(root).await?;
	match async {
	...
	.await
	{
		...
		Err(error) if error.code == "storage.disk_pressure" => {
			return crate::checkpoint::pressure::snapshot(root).await;
		}
	...
	tokio::time::timeout(
		...
		workspace::with_scratch(
			...
			async |scratch| {
				...
				let omitted_files = crate::checkpoint::omissions::inspect(
					root,
...
pub(crate) async fn patch(
	...
	.await
	{
		Ok(()) => {}
		Err(error) if error.code == "storage.disk_pressure" => {
			return Ok(crate::checkpoint::pressure::artifact());
		}
		Err(error) => return Err(error),
	}
	...
	if matches!(retention, Retention::Current) {
		...
		_file_lock =
			crate::artifact::files::publication_lock(core.run_home()).await?;
...
pub(crate) async fn files(
	...
	.collect();
	crate::checkpoint::omissions::include(root, &mut files, before, after)
		.await?;
	Ok(files)
}
