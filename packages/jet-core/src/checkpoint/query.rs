...
use crate::{
	...
	checkpoint::{
		capture as checkpoint_capture, change_artifact, state::missing,
	},
...
pub(crate) async fn query(
	...
	run_id: RunId,
	scope: DiffScope,
	start: crate::checkpoint::pages::Start,
	...
	if !matches!(scope, DiffScope::Turn { .. }) {
		...
		diff.artifact = if crate::checkpoint::pressure::incomplete(&diff.before)
			|| crate::checkpoint::pressure::incomplete(&diff.after)
		{
	...
	crate::checkpoint::pages::bound_page(core, &mut diff, start)?;
	diff.patch =
...
pub(crate) async fn next(
	...
	query(
		...
		page.run_id,
		page.scope.clone(),
		crate::checkpoint::pages::Start::After(Box::new(page)),
	)
	.await
}
