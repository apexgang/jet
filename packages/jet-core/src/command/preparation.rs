...
use crate::{
	...
	conversation::import::{self, DiscoveredConversation},
	error::CoreError,
	project::{self, Registrable},
	promotion::command::{self as promotion_command, PreparedPromotion},
	security::SecurityState,
	workspace::{self, PreparedWorkspace, WorkingTreeRequest},
};
...
pub(crate) enum Prepared {
	...
	CraftInstallation(crate::craft::installation::PreparedCraftInstallation),
	...
	Run(crate::run::command::LaunchPlan),
	...
	Fork(Box<crate::conversation::fork::PreparedFork>),
	/// An explicit package and independently pinned destination launch.
	Handoff(Box<crate::conversation::handoff::PreparedHandoff>),
	...
	async fn prepare(
		...
		match command {
			...
			Command::ChangeExtension { confirmation } => {
				crate::extension::work::validate(self, confirmation).await?;
				Ok(Prepared::Nothing)
			}
			Command::DisableCraft { .. } => Ok(Prepared::Nothing),
			Command::InstallCraft { confirmation } => {
				Ok(Prepared::CraftInstallation(
					crate::craft::installation::prepare(self, confirmation)
						.await?,
				))
			}
			Command::ApplyUserEdit {
			...
			Command::OpenTerminal {
				...
				crate::terminal::command::prepare(
					self,
					actor,
					*workspace_id,
					*rows,
			...
			Command::StartNoVisaRun(request) => Ok(Prepared::Run(
				crate::remote::no_visa_run::prepare(self, actor, request)
					.await?,
			)),
			Command::StartVisaRun(request) => Ok(Prepared::Run(
				crate::remote::visa::prepare(self, actor, request).await?,
			)),
			Command::StartRun {
				conversation_id,
				craft,
			...
			Command::ForkConversation {
				...
				crate::conversation::fork::prepare(
					self,
					*source_run_id,
					*checkpoint_turn,
				)
