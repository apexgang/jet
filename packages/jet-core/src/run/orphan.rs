...
use crate::{
	Actor, CommandId, CommandOutcome, Core, CoreError, RunId,
...
impl Core {
	...
	pub(crate) async fn orphaned_executions(
		...
		let mut executions: Vec<OrphanedExecution> = rows
			.iter()
			.map(|s| crate::run::state::decode(s))
			.collect::<Result<_, _>>()?;
		let next =
	...
	pub(crate) async fn prepare_execution_resolution(
		...
		let orphan: OrphanedExecution = crate::run::state::decode(&state)?;
		...
		if orphan.role == ExecutionRole::Terminal {
			return crate::terminal::orphan::prepare(self, request).await;
		}
...
impl EffectAdapter for Resolutions<'_> {
	async fn execute(&mut self, effect: &Effect) -> EffectResult {
		...
		{
			Ok(Some(json)) => {
				match crate::run::state::decode::<ExecutionResolution>(&json) {
					Ok(request) => request,
		...
		if self
			...
			.unwrap_or(false)
		{
			return crate::terminal::orphan::execute(self.0, &request).await;
		}
...
pub(crate) async fn settle(
	...
	if state == jet_store::EffectStateRecord::Completed {
		let json = tx
			...
			.await?
			.ok_or_else(unavailable)?;
		let request: ExecutionResolution = crate::run::state::decode(&json)?;
		if request.action != ExecutionAction::Leave {
			crate::terminal::orphan::settle(tx, &request, now).await?;
