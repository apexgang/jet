...
use crate::{
	...
	RunLifecycle, WorkingTree, conversation::fork::ForkLaunchContext,
	filesystem, project::repository, run::craft::PinnedCraft,
};
use jet_store::{
...
pub(crate) async fn record(
	...
	if tx.git_delivery_blocks(conversation_id.0).await? {
		return Err(crate::git_delivery::state::refused(
			"git.delivery_unresolved",
		));
	}
	crate::extension::work::admit_run(tx).await?;
	crate::craft::lifecycle::admit(tx, &plan.craft).await?;
	...
	let (mut queue, mut changes) = crate::turn::queue::prepare(
		tx,
		actor,
		command_id,
		conversation_id,
		crate::TurnSource::User,
		crate::turn::queue::Admission::Prompt(plan.prompt.clone()),
	)
	.await?;
	if !crate::schedule::work::can_dispatch(tx, conversation_id, &queue, now)
		.await?
	{
		return Err(CoreError::conflict(
			"turn.unresolved",
