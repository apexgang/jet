...
use crate::{
	...
	LaunchPlan, turn::queue as turn_queue,
};
...
async fn continuation(
	...
	if !crate::schedule::work::can_dispatch(tx, id, &queue, now).await? {
		return Ok(None);
	}
...
async fn prepare(
	...
	crate::auto_continue::work::claimed(tx, id, &mut queue, &entry, now)
		.await?;
