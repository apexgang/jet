...
use crate::{
	...
	RunId,
	review::{policy::Selection, unavailable},
};
...
impl Core {
	...
	pub(crate) async fn begin_automatic_review(
		...
		let Ok(turn) = self
			.store
			.read(async |tx| {
				crate::review::guard::turn(
					&crate::review::guard::load(tx, run_id).await?,
				)
			})
			.await
		else {
	...
	async fn review_approval(
		...
		let admission = self
			.store
			.write(async |tx| {
				let mut state = crate::review::guard::load(tx, run_id).await?;
				if crate::review::guard::turn(&state)? != turn {
		...
		} else if crate::review::action::operation(&request).is_none() {
	...
	async fn consult(
		...
		Ok(Reviewed {
			...
			model: pinned.model,
			verdict: crate::review::output::validate(&reply.output)?,
		})
	}
		...
		self.store
			.write(async |tx| {
				...
				let run = tx
					...
					.await?
					.ok_or_else(|| unavailable("review.run_unavailable"))?;
				if crate::review::guard::turn(&state).ok() != Some(turn)
					|| run.lifecycle != crate::RunLifecycle::Active
				{
					...
					if !crate::review::policy::unchanged(tx, review, &plan)
						.await?
					{
						review.outcome = ReviewOutcome::Unavailable {
...
impl Reviewed {
	...
	fn settled(self) -> Answered {
		let decision = crate::review::output::settle(&self.verdict);
		Answered {
