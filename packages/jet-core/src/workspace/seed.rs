...
//!
//! The capture itself lives in [`crate::workspace::seed_capture`]; this module holds
//! what the user selects and what the Workspace remembers.

use crate::{
	error::CoreError, filesystem::relative_path::RelativePath,
	workspace::seed_capture::CapturedSeed,
};
	...
	async fn all_eligible_changes_seed_the_workspace_and_leave_the_checkout_alone()
	 {
		...
		.unwrap();
		let snapshot =
			snapshot(&checkout.core, conversation.conversation_id).await;
	...
	async fn a_seed_that_cannot_be_taken_is_refused_without_a_trace() {
		...
		for (base, seed) in [
			...
			(
				...
				paths(&vec![
					"tracked.txt";
					crate::workspace::seed::MAX_SELECTED_PATHS + 1
				]),
			),
		] {
			let refusal =
				create(core, project_id, base, seed).await.unwrap_err();
