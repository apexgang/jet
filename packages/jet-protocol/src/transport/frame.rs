...
mod tests {
	...
	use super::{
		CONNECTION_STREAM, Frame, FrameError, FrameKind, FrameLimits,
		FrameReader, FrameWriter, MAX_CONTROL_FRAME, MAX_DATA_FRAME, StreamId,
	};
	...
	async fn oversized_data_declaration_is_rejected() {
		...
		header
			.extend_from_slice(&StreamId::new(1).unwrap().get().to_be_bytes());
		header.extend_from_slice(&declared.to_be_bytes());
		client.write_all(&header).await.unwrap();
