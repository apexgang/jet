...
mod error;
pub use error::{ErrorCategory, RecoveryAction, RestartMetadata, WireError};
...
mod tests {
	...
	use crate::conversation::workspace::{
		BaseSelection, SeedSelection, WorkingTreeRequest, Workspace,
		WorkspaceBase, WorkspaceSeed,
	};
	use crate::conversation::{
		CommandRequest, CommandResponse, ConflictState, Conversation,
		ConversationSnapshot, RetentionPolicy, RevisionConflict, Run,
		RunLifecycle,
	};
	use crate::pairing::{
	...
	use crate::transport::handshake::{ClientHello, ServerHello, VersionRange};
	use crate::{
	...
	fn sequences_and_revisions_round_trip_as_decimal_strings() {
		...
		let decoded: ClientMessage =
			decode_control(encoded.as_bytes()).unwrap();
	...
	fn only_canonical_decimal_strings_are_accepted_for_sequences() {
		...
		let rejected =
			["3", r#""""#, r#""+3""#, r#""03""#, r#""-1""#, r#""3.0""#]
				.map(|after| decode(after).map(|_| after).unwrap_err());

		assert_eq!(
			decode(r#""0""#).unwrap(),
			QueryRequest::Events { after: 0 }
		);
		assert!(
			rejected
				.iter()
	...
	fn only_lowercase_hexadecimal_of_the_fixed_width_is_a_key() {
		...
		let accepted =
			decode_control::<ClientMessage>(claim(lowercase).as_bytes());
		let refused = [
	...
	fn a_promotion_preview_has_the_agreed_wire_shape() {
		...
		let result = ServerMessage::QueryResult {
			id: 13,
			result: QueryResponse::PromotionPreview(Box::new(
				PromotionPreview {
					cursor: 3,
