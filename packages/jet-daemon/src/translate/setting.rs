...
fn introduced_in(key: SettingKey) -> u32 {
	match key {
		...
		| SettingKey::AutomaticReviewConsent => wire::AUTOMATIC_REVIEW_MINOR,
	}
}
...
pub(super) fn key(key: SettingKey) -> wire::SettingKey {
	match key {
		...
		SettingKey::AutomaticReviewBinding => {
			wire::SettingKey::AutomaticReviewBinding
		}
		SettingKey::AutomaticReviewConsent => {
			wire::SettingKey::AutomaticReviewConsent
		}
	}
}
...
mod tests {
	...
	use jet_core::{
		AuditBreach, AuditEpoch, AuditHead, AuditSequence, EventSequence,
		PlaneId, PlaneStatus, SecurityDegradation, SecurityState,
	};
	use jet_protocol as wire;
	use pretty_assertions::assert_eq;
	use uuid::Uuid;

	use crate::translate::{plane_status, setting};
