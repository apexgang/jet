...
mod compatibility;
use compatibility::{command_minor, query_minor, unsupported_minor};
...
async fn open(
	...
	let remote = preface == crate::remote::stdio::REMOTE_PREFACE;
	if remote {
		stream.read_exact(&mut preface).await.ok()?;
	}
	if preface != PREFACE {
...
impl Connection {
			...
			match decode_control(&payload) {
				...
				Err(_) => {
					...
					let response = tokio::spawn(async move {
						...
						crate::connection::pairing::enroll(
	...
	async fn serve_requests(
		...
		crate::connection::session::serve(
