...
async fn process_requests(
	...
	let mut terminals =
		crate::connection::terminal_stream::Terminals::new(data_limit);
	let mut artifacts =
		crate::connection::artifact_stream::Artifacts::new(data_limit);
	loop {
		...
		let (stream_id, payload, message) = match request {
			Request::Artifact { stream_id, control } => {
				let result = if terminals.contains(stream_id) {
					Err(crate::connection::artifact_stream::invalid())
				} else {
					artifacts
						.control(
		...
		if artifacts.contains(stream_id) {
			...
			send_stream_error(
				&replies,
				stream_id,
				crate::connection::artifact_stream::invalid(),
			)
			.await;
			continue;
		}
...
async fn reply_to(
	...
	match message {
		ClientMessage::RemoteTool { id, request } => {
			crate::remote::tool::answer(core, actor, minor, id, request).await
		}
