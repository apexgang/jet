...
impl TerminalHost for Terminals {
	fn discover(
		...
		Box::pin(crate::terminal_recovery::discover(home))
	}
	fn inspect(
		&self,
		home: PathBuf,
	...
	{
		Box::pin(crate::terminal_recovery::inspect(home, id))
	}
	fn terminate(
		&self,
		home: PathBuf,
		...
		Box::pin(crate::terminal_recovery::terminate(home, request))
	}
	fn start(
		&self,
		home: PathBuf,
...
async fn start(home: PathBuf, plan: TerminalPlan) -> Result<(), CoreError> {
	...
	tokio::task::spawn_blocking(move || {
		if crate::terminal_recovery::boot()? != boundary.boot
...
async fn operate(
	...
	let (descriptor, retained) = tokio::task::spawn_blocking(move || {
		if crate::terminal_recovery::boot()? != accepted.boot {
...
pub(crate) async fn exchange(
	...
	let descriptor: TerminalDescriptor =
		crate::run_host::receive(&mut reader).await?;
