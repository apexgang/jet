...
use crate::run::host::{
	self as run_host, CraftProcesses, RunConnection, filesystem,
};
use jet_core::{
...
pub(crate) async fn validate_boot(
	...
	let accepted = crate::run::craft::Contract::of(&plan.craft)
		.map_err(|_| RunRecoveryError::Unsafe)?
		.boot_identity;
...
pub(crate) async fn connect(
	...
	let contract = crate::run::craft::Contract::of(&plan.craft)
		.map_err(|_| RunRecoveryError::Unsafe)?;
	...
	Ok(Box::new(RunConnection {
		limits_subagents: crate::run::craft::Contract::of(&plan.craft)
			.map_err(|_| RunRecoveryError::Unsafe)?
			.limits_subagents(),
...
mod tests {
	...
	use crate::run::host::CraftProcesses;
	...
	async fn a_changed_boot_proves_loss_even_when_the_previous_pid_is_live() {
		...
		let plan = |boot: &str| LaunchPlan {
			model: None,
			child_work: jet_core::ChildWork::Native,
			visa: None,
			no_visa: None,
			turn_id: None,
